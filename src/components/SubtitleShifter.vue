<template>
  <div class="subtitle-shifter" ref="subtitleShifter">
    ...
    <p class="description">
      This is a simple project made with Vue.js to shift .srt files :
      <br />simply drop a file, adjust the timings and download the new generated file !
    </p>
...
export default {
  ...
  methods: {
    ...
    shiftSubtitle(timeStart, timeEnd, shiftTime, sub) {
      ...
      return this.convertToHMSMs(timeStart).concat(
        this.convertToHMSMs(timeEnd)
      );
    },
    shiftSubtitleLines(lines, h, m, s, ms, sub) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (result) {
          let timeStart = this.convertToMilliseconds(
              parseInt(result[1]),
              parseInt(result[2]),
              parseInt(result[3]),
              parseInt(result[4])
            ),
            timeEnd = this.convertToMilliseconds(
              parseInt(result[5]),
              parseInt(result[6]),
              parseInt(result[7]),
              parseInt(result[8])
          ...
          let shiftedValues = this.shiftSubtitle(
            timeStart,
            timeEnd,
            shiftTime,
            sub
