<template>
    <div class="number-input">
        ...
        <input :class="['number-input__field', {'number-input__field--large': large}]" type="text"
            ref="input"
            :value="value"
    ...
    methods: {
        formatValue(value) {
            ...
            value.toString().split('').forEach(char => {
                ...
                if (formattedValue > this.max || formattedValue < this.min || formattedValue.length > this.size) {
                    formattedValue = formattedValue.toString().substring(0, formattedValue.toString().length - 1);                    
                }
            });
